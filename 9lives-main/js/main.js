...
let app, auth, db;
let user = null; // Admin User
let communityUser = null; // Chat User
...
try {
    ...
    onAuthStateChanged(auth, (u) => {
        ...
        // Check for Admin Access (if already logged in)
        if (user && document.getElementById('admin-dashboard-view')) {
            // Check if user is admin (simplified check based on email)
            if(user.email.includes('admin') || user.email.includes('9lives')) {
                // If dashboard exists, unlock it
        ...
        // Check for Community Profile (Update UI)
        if (communityUser) {
...
function initHero3D() {
    ...
    // SHADER MATERIAL (Parallax + Glitch)
    const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            varying vec2 vUv;
            void main() {
                vec4 depthDistortion = texture2D(uDepth, vUv);
                float parallaxMult = depthDistortion.r;
                vec2 parallax = (uMouse) * parallaxMult * uVolo;
                float noise = sin(vUv.y * 80.0 + uTime * 6.0) * 0.0008;
                vec4 r = texture2D(uTexture, (vUv + parallax) + vec2(noise, 0.0));
                vec4 g = texture2D(uTexture, (vUv + parallax));
                vec4 b = texture2D(uTexture, (vUv + parallax) - vec2(noise, 0.0));
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        gsap.to(material.uniforms.uMouse.value, { x: mouse.x * 0.5, y: mouse.y * 0.5, duration: 1 });
...
// --------------------------------------------------------------------------
// COMMUNITY & CHAT LOGIC (WITH ANNOUNCEMENTS)
// --------------------------------------------------------------------------
function initCommunity() {
    ...
    // 2. Chat Feed Listener (Real-time)
        ...
        try {
            ...
            onSnapshot(q, (snap) => {
                ...
                msgs.forEach(data => {
                    ...
                    // Special styling for admin announcements
                    const containerClass = isAnnouncement 
                        ? "announcement-msg bg-red-900/20 p-3 rounded border-l-2 border-[#ff2a2a] mb-4" 
...
// --------------------------------------------------------------------------
// ADMIN SYSTEM (PIN + FIREBASE + ANNOUNCEMENTS)
// --------------------------------------------------------------------------
    ...
    if(addProdForm) {
        addProdForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                window.showToast("DEPLOY ERROR");
            }
        });
    }

    // 7. Post Announcement (NEW)
...
function renderStore() {
    ...
    onSnapshot(collection(db, "products"), (snap) => {
        grid.innerHTML = "";
        if(snap.empty) {
            grid.innerHTML = `<div class="col-span-full text-center font-mono text-gray-500">NO DROPS ACTIVE</div>`;
            return;
        }
        ...
        snap.forEach(doc => {
            ...
            el.innerHTML = `
                ...
                <div class="p-4 border-t border-white/10 flex justify-between items-start">
                    <div>
                        <h3 class="font-graffiti text-2xl">${escapeHtml(data.name)}</h3>
                        <p class="font-mono text-[10px] text-gray-500">LIMITED_EDITION</p>
                    </div>
...
function updateCartUI() {
    ...
    if(totalEl) totalEl.innerText = `$${cart.reduce((s,i)=>s+i.price,0).toFixed(2)}`;
    ...
    if(container) {
        ...
        cart.forEach((item, i) => {
            ...
            row.innerHTML = `
                <div>
                    <p class="font-mono font-bold text-sm">${escapeHtml(item.name)}</p>
                    <p class="text-xs text-gray-500">$${item.price}</p>
                </div>
                <button onclick="window.removeCartItem(${i})" class="text-[#ff2a2a] text-xs hover:text-white font-mono">[REMOVE]</button>
            `;
...
async function initPolls() {
    ...
    try {
        ...
        if(!docSnap.exists()) {
            await setDoc(pollRef, {
                options: ["CYBER_PINK_HOODIE", "VOID_BLACK_CARGO", "ACID_WASH_TEE"],
                votes_0: 420,
                votes_1: 380,
                votes_2: 150
            });
        ...
        console.warn("Poll init error:", e);
    }
    ...
    onSnapshot(pollRef, (docSnap) => {
        ...
        options.forEach((opt, i) => {
            ...
            html += `
            <div class="cursor-pointer group relative" onclick="window.votePoll(${i})">
                <div class="flex justify-between font-mono text-xs mb-1 text-gray-400 group-hover:text-[#ff2a2a]">
                    <span>${opt}</span>
                    <span>${percent}% (${votes})</span>
                </div>
                <div class="h-8 bg-[#111] border border-gray-800 relative overflow-hidden group-hover:border-[#ff2a2a]">
                    <div class="absolute top-0 left-0 h-full bg-[#ff2a2a] opacity-50 transition-all duration-1000" style="width: ${percent}%"></div>
                </div>
            </div>`;
        });
...
window.votePoll = async (idx) => {
    ...
    try {
        await updateDoc(doc(db, "polls", "active"), { [`votes_${idx}`]: increment(1) });
        window.showToast("VOTE REGISTERED");
    } catch(e) {
